# GitHub Copilot - Entwicklungs-Richtlinien

## 🎯 Projekt-Kontext

**Rechnungsverwaltungssystem** - GoBD-konforme Rechnungsstellung für kleine Unternehmen und Imker.

**Geschäftsmodell:**
- Direktverkauf (Endkunden): Normale Rechnungen
- Wiederverkäufer: Lieferscheine + Kommissionsware
- Markt/Hausverkauf: POS-System (Kasse)
- Steuern: §24 UStG für landwirtschaftliche Urproduktion (z.B. Honig: 7,8%)

**Stack:**
- Backend: Flask 3.0 + SQLAlchemy + PostgreSQL
- Frontend: Vanilla HTML/CSS/JavaScript (kein Framework)
- PDF: ReportLab
- Auth: Flask-Login + 2FA (pyotp)
- API: JWT-Tokens für PWA

---

## 📁 Projektstruktur

```
rechnungen/
├── app.py                      # Flask-App, alle Routes
├── models.py                   # SQLAlchemy-Modelle
├── config.py                   # Konfiguration (Firma, Steuern, Token)
├── email_service.py            # Flask-Mail Setup
├── .env                        # Secrets (NICHT committen!)
├── requirements.txt            # Python-Dependencies
├── GOBD_COMPLIANCE.md          # GoBD-Verfahrensdokumentation
├── .copilot-instructions.md    # Diese Datei
├── .gitignore                  # Git-Ignore-Rules
│
├── templates/                  # Jinja2-Templates
│   ├── base.html              # Base-Template (Navigation, User-Info)
│   ├── index.html             # Dashboard
│   ├── auth/                  # Login, 2FA
│   │   ├── login.html
│   │   └── verify_2fa.html
│   ├── invoices/              # Rechnungen
│   │   ├── list.html
│   │   ├── create.html
│   │   ├── edit.html
│   │   ├── view.html
│   │   └── create_cancellation.html
│   ├── customers/             # Kunden
│   ├── products/              # Produkte
│   ├── delivery_notes/        # Lieferscheine
│   └── pos.html               # Kasse (POS)
│
├── migrations/                # Datenbank-Migrationen
│   ├── create_users_table.py
│   ├── migrate_add_gobd_tables.py
│   └── regenerate_hashes.py
│
├── pdfs/                      # Generierte PDFs (GoBD-archiviert)
├── uploads/                   # Upload-Ordner
└── static/                    # CSS, JS, Bilder (falls vorhanden)
```

---

## 🗄️ Datenbank-Schema

### Kern-Entitäten

**User** (Authentifizierung)
- `id`, `username`, `email`, `password_hash`
- `role`: 'admin', 'cashier', 'reseller'
- `totp_secret`, `totp_enabled`, `backup_codes` (2FA)
- `api_token`, `api_token_expires` (PWA)
- `reseller_customer_id` (FK zu Customer, nur für role='reseller')

**Customer** (Kunden)
- `id`, `company_name`, `first_name`, `last_name`
- `email`, `phone`, `address`, `tax_id`

**Product** (Produkte)
- `id`, `name`, `quantity` (z.B. "500g"), `number` (Bestand)
- `price` (Endkunde), `reseller_price` (Wiederverkäufer)
- `tax_rate` (produktspezifisch, z.B. 7.8 für Honig)
- `lot_number`, `active`

**Invoice** (Rechnungen) - **KRITISCH für GoBD!**
- `id`, `invoice_number` (z.B. "RE-2024-11-08-0001", "STORNO-...", "BAR-...")
- `customer_id` (FK), `invoice_date`, `due_date`
- `status`: 'draft', 'sent', 'paid', 'cancelled'
- `subtotal`, `tax_rate`, `tax_amount`, `total`
- `tax_model`: 'standard', 'kleinunternehmer', 'landwirtschaft'
- `customer_type`: 'endkunde', 'wiederverkaeufer'
- `data_hash` (SHA-256, NOT NULL) - **Manipulationsschutz!**
- `notes`, `payment_method`

**LineItem** (Rechnungspositionen)
- `id`, `invoice_id` (FK), `product_id` (FK, optional)
- `description`, `quantity`, `unit_price`, `total`
- `tax_rate` (produktspezifisch)
- `position` (Sortierung)

**InvoiceStatusLog** (Audit-Trail) - **GoBD!**
- `id`, `invoice_id` (FK)
- `old_status`, `new_status`
- `changed_at`, `changed_by` (username!), `reason`

**InvoicePdfArchive** (PDF-Hashes) - **GoBD!**
- `id`, `invoice_id` (FK)
- `pdf_filename`, `pdf_hash` (SHA-256), `file_size`
- `created_at`, `archived_by` (username!)

### Wiederverkäufer-System

**DeliveryNote** (Lieferscheine)
- `id`, `delivery_note_number`, `customer_id` (FK)
- `delivery_date`, `status` ('delivered', 'partially_billed', 'billed')
- `show_tax`, `notes`

**DeliveryNoteItem** (Lieferschein-Positionen)
- `id`, `delivery_note_id` (FK), `product_id` (FK)
- `description`, `quantity`, `unit_price`, `total`

**ConsignmentStock** (Kommissionslager beim Wiederverkäufer)
- `id`, `customer_id` (FK), `product_id` (FK)
- `quantity` (aktueller Bestand beim Reseller)
- `unit_price` (Reseller-Preis)
- `last_delivery_note_id` (FK), `last_updated`
- **UNIQUE**: (customer_id, product_id)

### Weitere

**PaymentCheck** (Zahlungsprüfung)
- Automatische Abgleiche von Zahlungseingängen

**Reminder** (Mahnungen)
- Mahnstufen für überfällige Rechnungen

---

## 🔑 Naming Conventions

### Python (PEP 8)
- **Klassen**: `PascalCase` (z.B. `Invoice`, `LineItem`)
- **Funktionen/Methoden**: `snake_case` (z.B. `create_invoice`, `generate_hash`)
- **Variablen**: `snake_case` (z.B. `invoice_number`, `total_amount`)
- **Konstanten**: `UPPER_SNAKE_CASE` (z.B. `DEFAULT_TAX_RATE`)
- **Private**: `_leading_underscore` (z.B. `_calculate_internal`)

### Datenbank
- **Tabellen**: `snake_case` Plural (z.B. `invoices`, `line_items`)
- **Spalten**: `snake_case` (z.B. `invoice_number`, `created_at`)
- **FKs**: `[table]_id` (z.B. `customer_id`, `product_id`)
- **Indizes**: `idx_[table]_[column]` (z.B. `idx_invoices_status`)

### Templates
- **Dateien**: `snake_case.html` (z.B. `create_invoice.html`)
- **Ordner**: Singular (z.B. `invoices/`, `customers/`)
- **Base-Template**: `base.html`

### Routes
- **URL-Pattern**: `/resource` oder `/resource/<id>/action`
  - Liste: `/invoices`
  - Einzeln: `/invoices/<id>`
  - Aktion: `/invoices/<id>/cancel`
- **Funktionsnamen**: `action_resource` (z.B. `list_invoices`, `delete_customer`)

---

## 💼 Business-Logik

### Steuerberechnung

**3 Steuermodelle:**

1. **'standard'** - Normale MwSt.
   - MwSt. wird auf Netto aufgeschlagen
   - `total = subtotal + tax_amount`
   - Formel: `tax_amount = subtotal * (tax_rate / 100)`

2. **'kleinunternehmer'** - §19 UStG
   - Keine MwSt.
   - `total = subtotal`, `tax_amount = 0`
   - Hinweis auf PDF: "Kleinunternehmerregelung..."

3. **'landwirtschaft'** - §24 UStG (Durchschnittssatz)
   - **WICHTIG**: Brutto = Netto, MwSt. wird rückgerechnet
   - `total = subtotal`
   - Formel: `tax_amount = subtotal * (tax_rate / (100 + tax_rate))`
   - Standard-Satz: 7,8% (für Honig, Eier, etc.)

**Produktspezifische Steuersätze:**
- Jedes `Product` hat eigenes `tax_rate` Feld
- `LineItem.tax_rate` übernimmt vom Produkt
- Bei Berechnung: Verwende `item.tax_rate` statt `invoice.tax_rate`

### Rechnungsnummern

**Format:** `[PREFIX]-YYYYMMDD-####`

**Präfixe:**
- `RE-` - Normale Rechnung (Standard)
- `STORNO-` - Stornierungsrechnung
- `BAR-` - Barverkauf (POS/Kasse)

**Beispiele:**
- `RE-2024-11-08-0001` - Erste Rechnung am 8.11.2024
- `STORNO-20241108-0001` - Erste Storno am 8.11.2024
- `BAR-20241108-0042` - 42. Barverkauf am 8.11.2024

**Zähler:** Pro Präfix und Tag eigenständig

### Bestandsverwaltung

**Hauptbestand** (`Product.number`)
- Reduziert bei: Rechnung (sent), POS-Verkauf
- Erhöht bei: Stornierung, Entwurf-Löschung
- Methoden: `product.reduce_stock(qty)`, `product.increase_stock(qty)`

**Kommissionsbestand** (`ConsignmentStock.quantity`)
- Bestand beim Wiederverkäufer
- Erhöht bei: Lieferschein-Auslieferung
- Reduziert bei: Abrechnung, Stornierung
- Pro Reseller + Produkt eindeutig

**Bei Stornierung:**
```python
# 1. Hauptbestand zurück
product.increase_stock(abs(item.quantity))

# 2. Kommissionsbestand zurück (falls vorhanden)
if consignment_item:
    consignment_item.quantity += abs(item.quantity)
```

### Wiederverkäufer-Workflow

1. **Lieferschein erstellen**
   - Produkte auswählen, Mengen festlegen
   - Reseller-Preise verwenden
   - Status: 'delivered'

2. **Bestand aktualisieren**
   - Hauptbestand reduzieren: `product.number -= qty`
   - Kommissionsbestand erhöhen: `consignment_stock.quantity += qty`

3. **Abrechnung**
   - Rechnung aus Lieferschein erstellen
   - Nur verkaufte Menge abrechnen
   - Status: 'partially_billed' oder 'billed'

4. **Rückgabe**
   - Stornierung erstellt Gutschrift
   - Bestand zurückbuchen

---

## 🎨 Frontend-Patterns

### Flash-Messages

**Kategorien:**
- `success` - Grün (Erfolg)
- `danger` / `error` - Rot (Fehler)
- `warning` - Gelb (Warnung)
- `info` - Blau (Information)

**Verwendung:**
```python
flash('Rechnung erfolgreich erstellt!', 'success')
flash('Fehler: Nicht genug Bestand', 'danger')
flash('Achtung: Entwurf wird gelöscht', 'warning')
```

### Status-Badges

**Invoice Status:**
- `draft` → 🟠 Orange Badge
- `sent` → 🔵 Blau Badge
- `paid` → 🟢 Grün Badge
- `cancelled` → ⚫ Grau Badge

**Template:**
```html
<span class="badge badge-{{ invoice.status }}">
    {{ invoice.status|upper }}
</span>
```

### Formulare

**Standard-Struktur:**
```html
<form method="POST" action="{{ url_for('route_name') }}">
    <div class="form-group">
        <label for="field">Label</label>
        <input type="text" id="field" name="field" required>
    </div>
    <button type="submit" class="btn btn-primary">Speichern</button>
</form>
```

**Wichtig:**
- `required` für Pflichtfelder
- `name` Attribute für POST-Daten
- CSRF-Protection (automatisch in Flask)

### Navigation

**Haupt-Navigation** (`base.html`):
- Dashboard, Rechnungen, Neue Rechnung
- 🏪 Kasse (grün hervorgehoben)
- Lieferscheine, Kunden, Produkte, Bestand
- Zahlungen, Einstellungen
- **User-Info** (rechts oben): Username, Rolle, Abmelden

**Conditional Rendering:**
```html
{% if current_user.is_authenticated %}
    {% if current_user.has_role('admin') %}
        <!-- Nur für Admins -->
    {% endif %}
{% endif %}
```

---

## 🎯 Wichtige Workflows

### Rechnung erstellen

**Route:** `GET/POST /invoices/create`

**Ablauf:**
1. Kunde auswählen oder neu anlegen
2. Produkte hinzufügen (mit Menge)
3. Steuermodell wählen
4. `status = 'draft'` (noch änderbar/löschbar)
5. Totals berechnen
6. LineItems erstellen
7. **Hash generieren** (vor db.session.add!)
8. Status-Log erstellen
9. Speichern

**Code-Pattern:**
```python
invoice = Invoice(...)
line_items = [LineItem(...), ...]
invoice.line_items = line_items
invoice.calculate_totals()
invoice.generate_hash()  # VOR add()!

log = InvoiceStatusLog(
    invoice_id=invoice.id,
    old_status=None,
    new_status='draft',
    changed_by=current_user.username
)

db.session.add(invoice)
db.session.add(log)
db.session.commit()
```

### Rechnung versenden

**Route:** `POST /invoices/<id>/status` mit `new_status='sent'`

**Effekt:**
- ✅ Status → 'sent'
- ✅ Unveränderbarkeit greift
- ✅ 10-Jahre-Aufbewahrungspflicht
- ✅ Status-Log erstellt
- ❌ Keine Löschung mehr möglich

### Stornorechnung

**Route:** `GET/POST /invoices/<id>/cancel`

**Nur für:** status='sent' oder 'paid'

**Ablauf:**
1. Original-Rechnung laden
2. Neue Rechnung erstellen mit negativen Beträgen
3. Rechnungsnummer: `STORNO-YYYYMMDD-####`
4. LineItems kopieren (quantity * -1, total * -1)
5. Bestand zurückbuchen (Haupt + Kommission)
6. Original-Status → 'cancelled'
7. Beide Status-Logs erstellen
8. Hash für Storno generieren

### POS-Verkauf (Kasse)

**Route:** `GET /pos`, `POST /api/pos/complete_sale`

**Mobile-optimiert** (responsive Design)

**Ablauf:**
1. Produkte anzeigen mit Zähler (+/-)
2. Echtzeit-Bon-Vorschau (Datum, Uhrzeit, Summe)
3. "Verkauf abschließen" → AJAX-Request
4. Backend erstellt BAR-Rechnung
   - customer_id = "Barverkauf"-Kunde
   - status = 'paid' (sofort bezahlt)
   - payment_method = 'Barzahlung'
5. Bestand reduzieren
6. Hash generieren
7. Status-Log erstellen
8. Erfolg → Warenkorb leeren

### Entwurf löschen

**Route:** `POST /invoices/<id>/delete`

**Nur für:** status='draft'

**Ablauf:**
1. Validierung: `if invoice.status != 'draft': FEHLER`
2. Bestand zurückbuchen (LineItems)
3. Kommissionsbestand zurückbuchen (falls vorhanden)
4. LineItems löschen (CASCADE)
5. Status-Logs löschen (CASCADE)
6. Rechnung löschen

**Rechtfertigung:** Entwurf ist kein Geschäftsvorfall (GoBD)

---

**Alle Änderungen MÜSSEN GoBD-konform sein!** Siehe `GOBD_COMPLIANCE.md` für Details.

### Unveränderbarkeit (Immutability)

**Status-Workflow:**
```
draft → sent → paid
   ↓          ↓
DELETE    cancelled
```

**VERBOTEN:**
- ❌ Versendete Rechnungen (status='sent') löschen → NUR Stornierung erlaubt
- ❌ Bezahlte Rechnungen (status='paid') löschen → NUR Stornierung erlaubt
- ❌ Status zurücksetzen von 'sent' auf 'draft'
- ❌ Rechnungsdaten nach Status='sent' ändern

**ERLAUBT:**
- ✅ Entwürfe (status='draft') löschen
- ✅ Stornierung durch Stornorechnung (negative Beträge, Bestandsrückbuchung)
- ✅ Status vorwärts ändern: draft → sent → paid

### Audit Trail

**Bei JEDER Status-Änderung:**
```python
log_entry = InvoiceStatusLog(
    invoice_id=invoice.id,
    old_status=old_status,
    new_status=new_status,
    changed_by=current_user.username,  # IMMER aktuellen User verwenden!
    reason=reason
)
db.session.add(log_entry)
```

### Hash-Integrität

**Bei Rechnung-Erstellung/-Änderung:**
```python
# Pattern: Erst line_items zuweisen, DANN Hash generieren, DANN add/flush
invoice.line_items = line_items_list
invoice.generate_hash()
db.session.add(invoice)
db.session.flush()
```

**WICHTIG:** Hash MUSS vor `db.session.add()` generiert werden (wegen DB-Trigger)!

### PDF-Archivierung

**Beim ersten PDF-Download (status='sent'):**
```python
pdf_hash = hashlib.sha256(pdf_bytes).hexdigest()
archive = InvoicePdfArchive(
    invoice_id=invoice.id,
    pdf_filename=filename,
    pdf_hash=pdf_hash,
    file_size=len(pdf_bytes),
    archived_by=current_user.username
)
db.session.add(archive)
```

---

## 🔒 Authentifizierung & Autorisierung

### Rollen-System

**Rollen:**
- `admin`: Volle Rechte (alle Features)
- `cashier`: Nur Kasse + Rechnungen ansehen
- `reseller`: Eigener Bestand + eigene Preise (zukünftig)

**Route-Protection:**
```python
@app.route('/admin/users')
@login_required
@role_required('admin')
def manage_users():
    # Nur für Admins
```

**Kassen-Route (cashier + admin):**
```python
@app.route('/pos')
@login_required
@role_required('cashier', 'admin')
def pos():
    # Für Kassierer und Admins
```

### Audit-Trail mit User

**IMMER `current_user` verwenden statt "System":**
```python
# ✅ RICHTIG
log_entry.changed_by = current_user.username

# ❌ FALSCH
log_entry.changed_by = 'System'
```

---

## 📝 Git-Workflow

**Nach JEDER Änderung automatisch committen:**

```bash
git add .
git commit -m "[Beschreibung der Änderung]"
```

### Commit-Message-Format

**Feature/Neu:**
```
feat: [Kurze Beschreibung]

- Detail 1
- Detail 2
- GoBD: [Relevante Compliance-Info wenn anwendbar]
```

**Bugfix:**
```
fix: [Problem-Beschreibung]

- Was war das Problem
- Was wurde gefixt
- GoBD: [Falls betroffen]
```

**Datenbank/Migration:**
```
db: [Migration-Beschreibung]

- Neue Tabellen: [Liste]
- Geänderte Felder: [Liste]
- Migration-Script: [Dateiname]
```

**Authentifizierung/Sicherheit:**
```
auth: [Änderung]

- Details
- Betroffene Routen: [Liste]
```

**Beispiele:**
```bash
# Feature
git commit -m "feat: User-Management für Admins

- User-Liste mit Filter
- User erstellen/bearbeiten/deaktivieren
- Rollen-Zuweisung
- GoBD: changed_by verwendet jetzt current_user"

# Bugfix
git commit -m "fix: Hash-Generierung in complete_pos_sale

- Hash wurde NACH db.session.add() generiert
- Trigger verhinderte NULL-Werte
- Jetzt: Hash VOR add() generieren"

# Migration
git commit -m "db: User-Tabelle für Authentifizierung

- Neue Tabelle: users
- Felder: username, email, password_hash, role, totp_secret
- Migration: migrations/create_users_table.py
- Standard-Admin: admin/admin123"
```

---

## 🎨 Code-Style

### Python

**Flask-Routen:**
```python
@app.route('/endpoint', methods=['GET', 'POST'])
@login_required
@role_required('admin')
def my_route():
    """Docstring: Was macht diese Route"""
    if request.method == 'POST':
        # POST-Logik
        pass

    # GET-Logik
    return render_template('template.html')
```

**Datenbank-Operationen:**
```python
try:
    # Änderungen
    db.session.add(obj)
    db.session.commit()
    flash('Erfolgsmeldung', 'success')
except Exception as e:
    db.session.rollback()
    flash(f'Fehler: {str(e)}', 'danger')
    return redirect(url_for('fallback_route'))
```

**GoBD-Validierung:**
```python
# Status-Änderung validieren
if invoice.status == 'sent' and new_status == 'draft':
    flash('Fehler: Versendete Rechnungen können nicht zurück zu Entwurf (GoBD).', 'danger')
    return redirect(url_for('view_invoice', invoice_id=invoice.id))
```

### Templates

**Flash-Messages immer anzeigen:**
```html
{% with messages = get_flashed_messages(with_categories=true) %}
    {% if messages %}
        {% for category, message in messages %}
            <div class="flash {{ category }}">{{ message }}</div>
        {% endfor %}
    {% endif %}
{% endwith %}
```

**User-Info anzeigen:**
```html
{% if current_user.is_authenticated %}
    <span>👤 {{ current_user.username }}</span>
    <span class="badge">{{ current_user.role }}</span>
{% endif %}
```

---

## 🔧 Wichtige Patterns

### Hash-Generierung (GoBD)

```python
# KRITISCH: Reihenfolge beachten!
# 1. Invoice-Objekt erstellen
invoice = Invoice(...)

# 2. LineItems erstellen
line_items = [LineItem(...), LineItem(...)]

# 3. LineItems zuweisen
invoice.line_items = line_items

# 4. Hash generieren
invoice.generate_hash()

# 5. Zu Session hinzufügen
db.session.add(invoice)
db.session.flush()
```

### Status-Änderung mit Audit-Log

```python
def update_status(invoice, new_status, reason=None):
    old_status = invoice.status

    # Validierung (GoBD)
    if old_status == 'sent' and new_status == 'draft':
        raise ValueError('Unzulässige Status-Änderung (GoBD)')

    # Status ändern
    invoice.status = new_status

    # Audit-Log
    log_entry = InvoiceStatusLog(
        invoice_id=invoice.id,
        old_status=old_status,
        new_status=new_status,
        changed_by=current_user.username,
        reason=reason
    )
    db.session.add(log_entry)
    db.session.commit()
```

### Bestandsverwaltung

```python
# Bestand reduzieren (beim Verkauf)
if not product.reduce_stock(quantity):
    raise ValueError(f'Nicht genug Bestand: {product.name}')

# Bestand erhöhen (bei Stornierung)
product.increase_stock(quantity)
```

---

## 📚 Wichtige Dateien

| Datei | Zweck |
|-------|-------|
| `GOBD_COMPLIANCE.md` | **PFLICHTLEKTÜRE** - GoBD-Verfahrensdokumentation |
| `models.py` | Datenbankmodelle (User, Invoice, Product, etc.) |
| `app.py` | Flask-Routes, App-Factory |
| `config.py` | Konfiguration (Firma, Steuern, Token-Gültigkeit) |
| `.env` | Secrets (nicht committen!) |
| `migrations/` | Datenbank-Migrationen |
| `templates/` | Jinja2-Templates |
| `templates/auth/` | Login, 2FA |

---

## ⚠️ Was NIEMALS tun

- ❌ Versendete Rechnungen löschen
- ❌ Hash nach `db.session.add()` generieren
- ❌ Status zurücksetzen (sent→draft, paid→sent)
- ❌ `changed_by='System'` verwenden (immer `current_user.username`)
- ❌ Secrets in Code committen
- ❌ Änderungen ohne Git-Commit lassen
- ❌ Migrations ohne Test ausführen

---

## ✅ Best Practices

1. **Lies `GOBD_COMPLIANCE.md`** vor jeder Änderung an Rechnungen
2. **Teste Migrations** erst auf Entwicklungs-DB
3. **Flash-Messages** für User-Feedback
4. **Try-Except** bei DB-Operationen
5. **Audit-Logging** bei allen wichtigen Aktionen
6. **Git-Commit** nach jeder Änderung
7. **current_user** für Audit-Trail
8. **Route-Protection** mit `@login_required` und `@role_required`

---

## 🚀 Nächste Features (Roadmap)

- [ ] Route-Protection für alle Routen
- [ ] 2FA-Setup-Interface
- [ ] User-Management (Admin)
- [ ] JWT-API für PWA
- [ ] Progressive Web App (offline-fähig)
- [ ] Reseller-System (eigener Bestand + Preise)
- [ ] Automatische Backups

---

**Bei Unsicherheiten:** Frag nach! GoBD-Compliance ist kritisch für Finanzamt-Prüfungen.

---

## ✨ Implementierte Features (Stand 08.11.2025)

### Rechnungsverwaltung
- ✅ Rechnungen erstellen/bearbeiten/löschen (draft only)
- ✅ Status-Workflow (draft → sent → paid, cancelled)
- ✅ Stornorechnungen mit Bestandsrückbuchung
- ✅ PDF-Generierung (ReportLab)
- ✅ SHA-256 Hash-Schutz (GoBD)
- ✅ Audit-Trail (InvoiceStatusLog)
- ✅ PDF-Archivierung mit Hash (InvoicePdfArchive)
- ✅ Filter: Alle, Entwürfe, Versendet, Bezahlt, Stornorechnungen
- ✅ Fortlaufende Rechnungsnummern (pro Tag/Präfix)

### Kundenverwaltung
- ✅ Kunden anlegen/bearbeiten
- ✅ Kundentypen: Endkunde, Wiederverkäufer
- ✅ Adressverwaltung
- ✅ Kundenliste mit Suche

### Produktverwaltung
- ✅ Produkte anlegen/bearbeiten/deaktivieren
- ✅ Bestandsführung (Hauptlager)
- ✅ Reseller-Preise
- ✅ Produktspezifische Steuersätze
- ✅ Chargennummern

### Lieferscheine & Kommissionsware
- ✅ Lieferscheine erstellen
- ✅ Kommissionslager-Verwaltung
- ✅ Bestandstracking pro Reseller
- ✅ Abrechnung aus Lieferscheinen

### Point-of-Sale (Kasse)
- ✅ Mobile-optimierte Kassen-UI
- ✅ Echtzeit-Bon-Vorschau
- ✅ BAR-Rechnungen (sofort bezahlt)
- ✅ Touch-optimiert (45x45px Buttons)
- ✅ Live Datum/Uhrzeit
- ✅ GoBD-konform (Hash, Status-Log)

### Zahlungsmanagement
- ✅ Zahlungsprüfung (PaymentCheck)
- ✅ Mahnungen (Reminder)
- ✅ Zahlungseingänge abgleichen

### Authentifizierung & Sicherheit
- ✅ User-Modell (admin, cashier, reseller)
- ✅ Login/Logout (Flask-Login)
- ✅ Passwort-Hashing (Werkzeug)
- ✅ 2FA-Infrastruktur (TOTP, Backup-Codes)
- ✅ API-Token-Support (für PWA)
- ✅ Session-Management

### GoBD-Konformität
- ✅ Unveränderbarkeit (Hash-Protection)
- ✅ Audit-Trail (alle Status-Änderungen)
- ✅ PDF-Archivierung mit SHA-256
- ✅ Entwurfs-Löschung erlaubt
- ✅ Stornierung durch Gegenbuchung
- ✅ 10-Jahre-Aufbewahrung
- ✅ Vollständige Verfahrensdokumentation

### Noch NICHT implementiert (TODO)
- ⏳ Route-Protection (@login_required für alle Routen)
- ⏳ 2FA-Setup-Interface (QR-Code-Anzeige)
- ⏳ User-Management-UI (Admin-Panel)
- ⏳ JWT-API-Endpoints (für PWA)
- ⏳ Progressive Web App (PWA)
- ⏳ current_user in Audit-Logs (noch "System")
- ⏳ Reseller-Self-Service (eigener Login)
- ⏳ Email-Versand für Rechnungen
- ⏳ Automatische Backups

---

## 🔧 Entwicklungs-Workflow

### Lokale Entwicklung starten

```bash
# 1. Virtuelle Umgebung aktivieren
source venv/bin/activate  # Linux/Mac
# oder: venv\Scripts\activate  # Windows

# 2. Abhängigkeiten installieren
pip install -r requirements.txt

# 3. Umgebungsvariablen setzen (.env)
# DATABASE_URL, SECRET_KEY, COMPANY_*, BANK_*

# 4. Datenbank migrieren (falls nötig)
python migrations/create_users_table.py

# 5. App starten
python app.py
# oder: flask run --port 5000
```

### Datenbank-Migrationen erstellen

**Pattern:**
1. `models.py` ändern
2. Migrations-Skript erstellen: `migrations/add_[feature].py`
3. App-Context verwenden: `app = create_app()`
4. `db.create_all()` für neue Tabellen
5. Bestehende Daten migrieren (falls nötig)
6. Testen!
7. Git-Commit

**Beispiel:**
```python
#!/usr/bin/env python3
from app import create_app
from models import db, NewModel

def migrate():
    app = create_app()
    with app.app_context():
        db.create_all()
        print("✓ Migration erfolgreich")

if __name__ == '__main__':
    migrate()
```

### Testing

**Aktuell:** Manuelles Testing
- Login testen (admin/admin123)
- Rechnung erstellen
- PDF generieren
- Hash verifizieren
- Storno testen
- POS testen

**Zukünftig:** Unit-Tests + Integration-Tests

---

## 🐛 Debugging-Tipps

### Hash-Integritätsfehler

**Problem:** "Hash stimmt nicht überein"

**Ursachen:**
1. Hash wurde NACH `db.session.add()` generiert
2. LineItems wurden nach Hash-Generierung geändert
3. Datenbank-Manipulation

**Lösung:**
```python
# RICHTIG:
invoice.line_items = items_list
invoice.calculate_totals()
invoice.generate_hash()  # VOR add()!
db.session.add(invoice)

# FALSCH:
db.session.add(invoice)
invoice.generate_hash()  # Zu spät! Trigger verhindert Update
```

### "NULL value in column data_hash"

**Problem:** Database-Trigger `protect_invoice_hash()` verhindert NULL

**Lösung:** Hash VOR `db.session.add()` generieren (siehe oben)

### Status-Änderung wird abgelehnt

**Problem:** "Versendete Rechnungen können nicht zurück zu Entwurf"

**Erklärung:** GoBD-Regel - BEABSICHTIGT!

**Lösung:** Stornierung verwenden statt Status-Rücknahme

### Bestand wird nicht reduziert

**Problem:** `product.number` ändert sich nicht

**Prüfen:**
1. Wird `product.reduce_stock(qty)` aufgerufen?
2. Gibt Funktion `False` zurück? (Nicht genug Bestand)
3. Wurde `db.session.commit()` vergessen?

**Debug:**
```python
if not product.reduce_stock(quantity):
    print(f"Fehler: Bestand nicht ausreichend! {product.number} < {quantity}")
    db.session.rollback()
```

### Login funktioniert nicht

**Prüfen:**
1. User existiert? `User.query.filter_by(username='admin').first()`
2. Passwort korrekt? `user.check_password('admin123')`
3. User aktiv? `user.is_active == True`
4. Flask-Login initialisiert? `login_manager.init_app(app)`

---

## 📚 Konfiguration (config.py / .env)

### Pflicht-Variablen

```bash
# .env Beispiel
SECRET_KEY=your-secret-key-here-change-in-production
DATABASE_URL=postgresql://user:password@localhost/rechnungen

# Firma
COMPANY_NAME=Imkerei Mustermann
COMPANY_HOLDER=Max Mustermann
COMPANY_STREET=Bienenweg 42
COMPANY_ZIP=12345
COMPANY_CITY=Honigstadt
COMPANY_EMAIL=info@imkerei-mustermann.de
COMPANY_PHONE=+49 123 456789
COMPANY_TAX_ID=DE123456789

# Bank
BANK_NAME=Sparkasse Honigstadt
BANK_IBAN=DE00 0000 0000 0000 0000 00
BANK_BIC=SPARKDE01

# Optional
PAYPAL=paypal@imkerei-mustermann.de
DEFAULT_TAX_RATE=19.00
LANDWIRTSCHAFTLICHE_URPRODUKTION_TAX_RATE=7.80
API_TOKEN_EXPIRY_DAYS=30
```

### Steuerung über Config

**Zugriff in Code:**
```python
from flask import current_app

company_name = current_app.config['COMPANY_NAME']
tax_rate = current_app.config['DEFAULT_TAX_RATE']
token_days = current_app.config['API_TOKEN_EXPIRY_DAYS']
```

---

## 🎓 Lern-Ressourcen

**GoBD:**
- `GOBD_COMPLIANCE.md` (vollständige Dokumentation)
- BMF-Schreiben 28.11.2019
- IHK-Leitfäden

**Flask:**
- https://flask.palletsprojects.com/
- Flask-Login: https://flask-login.readthedocs.io/
- SQLAlchemy: https://docs.sqlalchemy.org/

**Steuerrecht (Deutschland):**
- §24 UStG - Durchschnittssätze Land- und Forstwirtschaft
- §19 UStG - Kleinunternehmer
- GoBD - Ordnungsmäßige Buchführung

---
