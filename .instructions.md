...
- Never, never use f-string in log-statements (no logging-fstring-interpolation)

## Naming Conventions (PEP 8)

- Use `PascalCase` for class names (e.g., `Invoice`, `CustomerService`)
- Use `snake_case` for functions, methods, and variables (e.g.,
...
- Database tables: lowercase with underscores (e.g., `invoice_items`,
  `customers`)
- **Database columns**: Same as class attributes - `snake_case` (SQLAlchemy
  auto-maps)
- Flask routes: kebab-case URLs (e.g., `/invoice/create`, `/api/customer-list`)

### Boolean Naming

- Prefix with `is_` for state queries (e.g., `is_active`, `is_valid`,
  `is_authenticated`)
- Prefix with `has_` for possession/existence (e.g., `has_permission`,
  `has_totp`, `has_errors`)
- Prefix with `can_` for capabilities/permissions (e.g., `can_edit`,
  `can_delete`, `can_access`)
- Prefix with `should_` for recommendations (e.g., `should_retry`,
  `should_validate`)
- Prefix with `needs_` for requirements (e.g., `needs_update`, `needs_review`)
- Avoid double negations (e.g., use `is_deleted = False` instead of
  `is_not_deleted = True`)
...
- Flash user-friendly messages with appropriate categories ('error', 'warning',
  'success')
...
- Use conventional commits: `feat:`, `fix:`, `chore:`, `docs:`, `style:`,
  `refactor:`
...
- Write factual statements and direct commands. Avoid hypotheticals like "could"
  or "would".
