...
class CrowdSecApp:
    ...
    def _log_error_response(self, response):
        ...
        self.logger.warning(
            f'{ip} - "{method} {path}" {status} - UA:"{user_agent}" Ref:"{referrer}"'
        )
    ...
    def log_failed_login(self, username, ip=None, reason='invalid_credentials'):
        ...
        self.logger.warning(
            f'FAILED_LOGIN ip={ip} user={username} reason={reason} '
    ...
    def log_unauthorized_access(self, resource, required_role='', ip=None):
        ...
        self.logger.warning(
            f'UNAUTHORIZED_ACCESS resource={resource} required_role={required_role} '
