...
def generate_delivery_note_pdf(delivery_note, pdf_folder, config=None):
    ...
    details_values = [delivery_note.delivery_note_number, delivery_note.delivery_date.strftime("%d.%m.%Y")]

    details_table = Table([details_headers, details_values], colWidths=[90 * mm, 80 * mm])
    ...
    if delivery_note.show_tax:
        ...
        totals_table = Table(totals_data, colWidths=[10 * mm, 60 * mm, 15 * mm, 25 * mm, 15 * mm, 25 * mm, 25 * mm])
    ...
    signature_table = Table(
        [["_" * 30, "_" * 30], ["Datum, Unterschrift Lieferant", "Datum, Unterschrift Empfänger"]],
