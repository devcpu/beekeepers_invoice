...
from models import ConsignmentStock, Customer, Product, User, db
...
with app.app_context():
    ...
    for stock_data in type3_stock_data:
        ...
        if not stock:
            ...
            print(
                f"✅ ConsignmentStock: {stock_data['product'].name} → {type3_customer.company_name} ({stock_data['quantity']} Stk @ {stock_data['unit_price']}€)"
            )
        else:
    ...
    for stock_data in owner_stock_data:
        stock = ConsignmentStock.query.filter_by(customer_id=marktstand.id, product_id=stock_data["product"].id).first()
